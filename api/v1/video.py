...
async def upload_video(file: UploadFile = File(...)):
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Failed to create upload directories")
        ...
        try:
            ...
            logger.error(f"File save failed for {original_path}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to enqueue task for {session_id}: {e}", exc_info=True)
...
def get_results(session_id: str):
    try:
        ...
        try:
            ...
            logger.error(f"Failed to parse results for {session_id}: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail="Invalid results format")
        except IOError as e:
            logger.error(f"Failed to read results for {session_id}: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail="Failed to read results")
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error getting results for {session_id}: {e}", exc_info=True)
