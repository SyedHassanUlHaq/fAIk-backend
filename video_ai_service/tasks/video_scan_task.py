"""
Celery task that runs the video deepfake AI model on a queued scan.
...
Flow:
  ...
  4. Run the video deepfake model (RAFT + XCLIP-DeMamba fused head).
...
from config.project_config import THRESHOLD
...
def _process_video(scan: Scan, video_path: str, db) -> dict:
    ...
    result_type = "deepfakeVideo" if verdict == "ai" else "authentic"
...
@celery_app.task(name="tasks.process_video_scan", bind=True, max_retries=2)
def process_video_scan(self, scan_id: str):
    db = SessionLocal()
    tmp_path = None
    ...
    try:
        ...
        verdict_label = {"ai": "AI-generated", "authentic": "Authentic"}.get(scan.verdict, "Complete")
        send_push(
